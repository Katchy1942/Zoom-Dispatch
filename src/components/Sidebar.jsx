		...
		<div className="md:hidden flex items-center">
			...
			{createPortal(
				<>
					...
					<div className={`fixed top-0 right-0 h-dvh w-72 bg-[#070707] z-50 transform transition-transform duration-300 flex flex-col pt-20 px-6 ${isOpen ? 'translate-x-0' : 'translate-x-full'} border-l border-[#FF0000]/20`}>
						<button onClick={() => setIsOpen(false)} className="absolute top-6 right-6 text-[#F4F4F4] cursor-pointer">
							<X size={28} />
						</button>

						<ul className="flex flex-col gap-6 text-[#F4F4F4] text-[18px] font-medium">
							{navLinks.map((link) => (
								<li key={link.name}>
									{link.name !== "Partner With Us" ? (
										...
										<div className="flex flex-col gap-4">
											...
											<div className={`flex flex-col gap-4 pl-4 overflow-hidden transition-all duration-300 ${partnerOpen ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'}`}>
												...
												<Link to="/businesses" className="text-base text-[#F4F4F4]/70 hover:text-[#FF0000] transition-colors" onClick={() => setIsOpen(false)}>
													Businesses
												</Link>
											</div>
										</div>
